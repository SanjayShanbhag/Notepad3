...
<html>
	<head>
        ...
        <script type="text/javascript">
            ...
            var url = "getPublicData.php";
            if(navigator.onLine){
                syncing();
                $.post(url,{id: id1}, function(data){
                    $('.cont').append(data);
                });
            }else{
                var db;
                ...
                request1.onsuccess = function (evt) {
                    db = request1.result;
                    var transaction = db.transaction(["publicNotesOffline"]);
                    var objectStore = transaction.objectStore("publicNotesOffline");
                    ...
                    request.onsuccess = function(event) {
                        ...
                        if(request.result) {
                            ...
                            document.getElementById('submitoffline').style.display = 'none';
                            var y = document.createTextNode('This Note Is Not Available Offline.');;
                            x.appendChild(y);
                            var div = document.getElementById('cont');
                            div.appendChild(x);
                        }
            ...
            function editsubmit(){
                ...
                id1 = id1.split("=");
                id1 = id1[1];
                var url = "editpnoteprocess.php";
                if(navigator.onLine){
                    $.post(url,{title: titl, content: cont, id: id1}, function(data){
                        var x = document.getElementById("snackbar");
                    ...
                    var x = document.getElementById("snackbar");
                    x.innerHTML = "You are offline. Please go online to save.";
                    x.className = "show";
                    setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000);
                }
            }
            ...
            function saveOffline(){
                ...
                id1 = id1[1];
                var content1 = document.getElementById('dcontent').innerHTML;
                content1 = encodestring(content1);
                var title1 = document.getElementById('dtitle').innerHTML;
                title1 = encodestring(title1);
                var addtime1 = document.getElementById('ddate').innerHTML;
                addtime1 = encodestring(addtime1);
                var db5;
                var request5 = indexedDB.open("Notepad3",1);
                request5.onsuccess = function (evt) {
                    db5 = request5.result;
                    var request6 = db5.transaction(["publicNotesOffline"], "readwrite")
                    .objectStore("publicNotesOffline")
                    .put({ title: title1, content: content1, noteid: id1, addtime: addtime1 });
                    var x = document.getElementById("snackbar");
                    x.innerHTML = "Note has been saved. This note can now be viewed offline";
                    x.className = "show";
                    setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000);
                    document.getElementById('submitsave').style.display = 'none';
                    document.getElementById('submitoffline').style.display = 'block';
                }
            }
            ...
            var db6;
            var request6 = indexedDB.open("Notepad3",1);
            request6.onsuccess = function (evt) {
                db6 = request6.result;
                var objectStore6 = db6.transaction("publicNotesOffline").objectStore("publicNotesOffline");
                objectStore6.openCursor().onsuccess = function(event) {
                     var cursor6 = event.target.result;
                    if (cursor6) {
                        var noteid = cursor6.value.noteid;
                        if(noteid == id1){
                            document.getElementById('submitsave').style.display = 'none';
                            document.getElementById('submitoffline').style.display = 'block';
                        }
        ...
        <div class="w3-main" style="margin-left:250px">
            ...
            <div class="container">
                ...
                <div style="display: flex; justify-content: center;">
                    <button class="btn btn-primary" onclick="saveOffline()" id="submitsave">Save for Offline</button>
                    <button class="btn btn-primary" onclick="saveOffline()" id="submitoffline" style="display: none;">Update Offline Version</button>
                </div>
                <div class="edit" id="edit" style="display: none;">
                    ...
                    <br>
                    <div style="display: flex; justify-content: center;">
                        <button class="btn btn-primary" onclick="editsubmit()" id="submitsave1">Save</button>
                    </div>
                </div>
                <div class="fab" id = 'fab' onclick="editactivate()"> <i class="fa fa-edit" style="margin-left: 5px;"></i> </div>
                <div id="snackbar"></div>
            </div>
            <br><br>
            <div style="display: flex; justify-content: center;">
